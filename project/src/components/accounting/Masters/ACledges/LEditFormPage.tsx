    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-gray-100 p-4">
      <div className="fixed top-[10%] bg-white p-8 rounded-xl shadow-lg w-full max-w-4xl border border-blue-500 relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-gray-600 text-sm font-medium mb-2">Parent (Group)</label>
            <select
              name="group"
          ...
          {["ledgerCode", "ledgerName", "shortName", "ledgerType", "shortCode", "glReference"].map((name, index) => (
            <div key={name} className="">
              <label className="block text-gray-600 text-sm font-medium mb-2">{name.replace(/([A-Z])/g, ' $1').trim()}</label>
              <input
                type="text"
                name={name}
                value={formData[name]}
